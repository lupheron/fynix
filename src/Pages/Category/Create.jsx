...
function Create({ isCreating, handleClose, handleCreate }) {
    ...
    return (
        <div>
            <Modal width={800} open={isCreating} onCancel={handleClose} footer={false} title="Yangi Kategoriya Qo'shish">
                <Form
                    onFinish={(values) => {
                        handleCreate(values);
                        form.resetFields();
                ...
                >
                    <Row gutter={16}>
                        <Col span={8}>
                            <Form.Item name="name" label="Kategoriya nomi" rules={[{ required: true, message: "Kategoriya nomini kiriting!" }]}>
                                <Input type="text" />
                            </Form.Item>
                        </Col>
                    </Row>
